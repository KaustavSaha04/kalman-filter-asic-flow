...
int equal_matrix(Matrix *a, Matrix *b, k_data_t tolerance);
...
void update(KalmanFilter *f, int N) {
    VITIS_LOOP_38_1: for (int i = 0; i < N; ++i) {
...
__attribute__((sdx_kernel("kalman_hw_update", 0))) void kalman_hw_update(k_data_t obs_pos, k_data_t *est_pos, k_data_t *est_vel) {
#pragma HLS TOP name=kalman_hw_update
# 46 "kalman.cpp"

#pragma HLS reset variable=hw_filter off
#pragma HLS reset variable=is_initialized off
#pragma HLS ARRAY_PARTITION variable=hw_filter complete recursive
#pragma HLS INTERFACE s_axilite port=return bundle=CTRL register
#pragma HLS INTERFACE s_axilite port=obs_pos bundle=CTRL register
#pragma HLS INTERFACE s_axilite port=est_pos bundle=CTRL register
#pragma HLS INTERFACE s_axilite port=est_vel bundle=CTRL register
...
#pragma HLS INLINE
 ...
 multiply_matrix(&f->observation_model, &f->predicted_state,
  ...
  invert_1x1(&f->innovation_covariance, &f->inverse_innovation_covariance);
